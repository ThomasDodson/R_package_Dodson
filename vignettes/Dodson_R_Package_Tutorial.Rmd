...
```{r}
download.file(url= "https://raw.githubusercontent.com/ThomasDodson/R_package_Dodson/master/data/SH16_Mussels.csv", destfile = "data/SH_Mussels.csv")
```
...
```
## LinearModelWidth 
LinearModelWidth takes in a dataframe of mussel metrics and 2 variables from within the dataframe, one of which is meant to be width. It then generates a linear model for analysis.

Example: LinearModelWidth(Mussel_data, Mussel_data$Variable_1, Mussel_data$Variable_2)

Example with test data examining variables width and depth:
LinearModelWidth(SH16_Mussels, SH16_Mussels$Width, SH16_Mussels$Depth)
